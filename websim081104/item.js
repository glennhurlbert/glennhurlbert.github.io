...
function Item(params) {
    ...
    if (params.constructor === Item) {
        ...
        this.bases = [];
        for (i = 0; i < params.bases.length; i++) {
            this.bases[i] = params.bases[i];
        }
    } else {
        // parse string params
        ...
        this.basicVar = (params.basicVar !== undefined ? parseInt(params.basicVar, 10) : 1);
        ...
        // [Number]:numRows. ??? something with pivoting
        this.bases = (params.bases !== undefined ?
            splitAndCast(params.bases, ",", Number) :
            repeatToArray(0, this.tableau.length));
    }

    // init members
    if (params.basicVar === undefined) {
        this.findBases();
        this.getBasicVar();
    }
...
Item.prototype.findBases = function() {
    for (var j = 0; j < this.tableau[0].length; j++) { // iterate over cols
        var nonzero_vals = 0; // count non-zero vals in this col
        var nonzero_row = -1; //
...
Item.prototype.updateTableau = function() {
    ...
    for (var i = 0; i < (this.tb.childNodes.length - 1) / 2; i++) {
        ...
        for (var j = 0; j < (this.tb.childNodes[i].childNodes.length - 1) / 2; j++) {
            t[i][j] = parseInt(this.tb.childNodes[i * 2 + 1].childNodes[j * 2 + 1].firstChild.value, 10);
        }
    }
    this.tableau = t;
};
...
Item.prototype.createTb = function() {
    ...
    for (j = 0; j < this.tableau[0].length; j++) {
...
Item.prototype.addColumnBefore = function(c) {
    ...
    for (i = 1; i < this.tb.childNodes.length; i++) {
...
Item.prototype.addRowBefore = function(r) {
    ...
    for (j = 1; j < (this.tb.childNodes[0].childNodes.length + 1) / 2; j++) {
...
Item.prototype.getBasicVar = function() {
    this.basicVar = 1;
    if (this.bases.length == this.tableau.length) {
