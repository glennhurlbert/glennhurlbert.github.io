...
function copyTableauNewWindow() {
   var s = activeItem.tableau[0].join(",");

   for (var i = 1; i < activeItem.tableau.length; i++) {
      s += "," + activeItem.tableau[i].join(",");
   }
   window.open(getBaseURL(window.location.href) + "?title=" + title + chars[counter++] + "&tableau=" + s + "&numRows=" + activeItem.tableau.length + "&numCols=" + activeItem.tableau[0].length);
}
...
function newTableau(tableau, type, equalRows, freeVars) {
   ...
   activeItem = new Item(tableau,type,0,1,freeVars,equalRows);
   m.appendChild(activeItem.div);
   scroll(0,0);
}
...
function convertToLaTeX() {
   ...
   for (var j = 1; j < activeItem.tb.childNodes[0].childNodes.length; j++) {
      if (j % 2 == 0) {
   ...
   for (var i = 1; i < activeItem.tb.childNodes.length; i++) {
      if (i % 2 == 0) {
         ...
         for (var j = 3; j < activeItem.tb.childNodes[i].childNodes.length; j = j + 2) {
